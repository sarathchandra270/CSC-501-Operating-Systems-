...
void test2 ()
{
	...
	rd4 = create(reader2, 2000, 20, "reader2", 3, 'E', lck, 20);
		...
		wr2 = create(writer2, 2000, 20, "writer2", 3, 'F', lck, 25);
...
void test3 ()
{
        ...
        resume(rd1);
        sleep (1);
	assert (getprio(wr1) == 25, "Test 3failed");

        kprintf("-start reader B, then sleep 1s. reader B(prio 30) blocked on the lock\n");
        resume (rd2);
	sleep (1);
	assert (getprio(wr1) == 30, "Test 3.2 failed");
	
	kprintf("-kill reader B, then sleep 1s\n");
	kill (rd2);
	sleep (1);
	assert (getprio(wr1) == 25, "Test 3.3 failed");

	kprintf("-kill reader A, then sleep 1s\n");
	kill (rd1);
	sleep(1);
	assert(getprio(wr1) == 20, "Test 3.4 failed");
