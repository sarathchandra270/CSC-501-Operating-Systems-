...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	createPageDirectory(NULLPROC);
	//kprintf("\n%d",proctab[NULLPROC].pdbr);
...
void createGlobalPageTables()
{
	...
	for(i =0; i<4; i++)
	{
		int f_no = -1;
		get_frm(&f_no);
		kprintf("frame no PT: %d\n",f_no);
		frm_tab[f_no].fr_status = FRM_MAPPED;
		frm_tab[f_no].fr_pid = NULLPROC;
		frm_tab[f_no].fr_type = FR_TBL;
		
		page_ptr = (pt_t *)((FRAME0+f_no) * NBPG);
		...
		for(j=0;j<1024;j++)
		{
			...
			page_ptr->pt_dirty = 0;
			page_ptr->pt_mbz = 0;
			page_ptr->pt_global = 1;
			page_ptr->pt_avail = 0;
			//putting frame number since address is 32 bit but pt_base is 20bit
			page_ptr->pt_base = FRAME0*i + j; 
			page_ptr++;
		}
	}
}

void createPageDirectory(int pid)
{
	int f_no =-1;
	int i;
	pd_t *directory_ptr;
	get_frm(&f_no);
	kprintf("frame no PD: %d\n",f_no);
		frm_tab[f_no].fr_status = FRM_MAPPED;
		frm_tab[f_no].fr_pid = pid;
		frm_tab[f_no].fr_type = FR_DIR;
		
		proctab[pid].pdbr = (FRAME0 + f_no) * NBPG;
		
		directory_ptr = (pd_t *)((FRAME0+ f_no) * NBPG);
		for(i = 0; i < 1024; i++)
		{
			directory_ptr->pd_pcd = 0;
			directory_ptr->pd_acc = 0;
			directory_ptr->pd_mbz = 0;
			directory_ptr->pd_fmb = 0;
			directory_ptr->pd_global = 0;
			directory_ptr->pd_avail = 0;
			directory_ptr->pd_write = 1;
			directory_ptr->pd_user = 0;
			directory_ptr->pd_pwt = 0;
			if(i<4)
			{
				directory_ptr->pd_pres = 1;
				directory_ptr->pd_base = FRAME0+i;
			}
			else
			{
				directory_ptr->pd_pres = 0;
				directory_ptr->pd_base = 0;
			}	
			directory_ptr++;
		}
}
