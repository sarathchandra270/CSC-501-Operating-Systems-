...
SYSCALL get_frm(int* avail)
{
	...
	if(page_replace_policy == SC)
	{
		...
		kprintf("Replacing Frame No: %d With SC Policy\n",f_sc);
		free_frm(f_sc);
		*avail = f_sc;
		restore(ps);
		return(OK);
	...
	if(page_replace_policy == LFU)
	{
		...
		kprintf("Replacing Frame No: %d With LFU Policy\n",f_lfu);
		free_frm(f_lfu);
		*avail = f_lfu;
		restore(ps);
		return(OK);
...
int getFrameByLFU()
{
	...
	for(i=0;i<1024;i++)
	{
		...
		{
			if(frm_tab[i].fr_counter < minCount)
			{
				minCount = frm_tab[i].fr_counter;
				index = i;
			}
			else if(frm_tab[i].fr_counter == minCount)
			{
				...
				{
					minCount = frm_tab[index].fr_counter;
				}
			}
			else
			{
