...
void proc1_test6(int *ret) {
	...
	for(i = 0; i < MAX_BSTORE; i++){
		...
		vpno = tempaddr >> 12;
		kprintf("xcoming herre\n");

		get_bs(i, 100);
kprintf("came here also\n");
...
kprintf("after release<MMEEE\n");
		for (j = 0; j < 100; j++) {
kprintf("after release UNMAP\n");
			*(vaddr + j * NBPG) = 'A' + i;
...
kprintf("\nTest 6: Stress testing 1\n");

	resume(pid1);
kprintf("\nTest 6: Stress testing 2\n");
	sleep(4);

	kill(pid1);
kprintf("\nTest 6: Stress testing 3\n");
	if (ret != TPASSED)
